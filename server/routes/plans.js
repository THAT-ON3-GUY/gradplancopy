...
router.get('/:id/courses', (req, res) => {
  ...
  const rows = db
    .prepare(
      `SELECT pc.id as plan_course_id, pc.semester_year, pc.semester_term, pc.planned_credits,
              c.id as course_id, c.code, c.name, c.credits, c.department, c.description,
              c.prerequisites, c.offered, c.is_variable_credit
  ...
  res.json(
    rows.map((r) => ({
      ...
      planned_credits: r.planned_credits,
      course: {
        id: r.course_id,
        code: r.code,
        name: r.name,
...
// PATCH /api/plans/:id/courses/:pcId — move course or adjust credits
router.patch('/:id/courses/:pcId', (req, res) => {
  const db = getDb();
  const { semester_year, semester_term, planned_credits } = req.body;
  ...
  if (planned_credits !== undefined) {
    db.prepare(
      'UPDATE plan_courses SET planned_credits = ? WHERE id = ? AND plan_id = ?'
    ).run(planned_credits, req.params.pcId, req.params.id);
  }
...
// GET /api/plans/:id/transfer-credits — get transfer credits
router.get('/:id/transfer-credits', (req, res) => {
  const db = getDb();
  const rows = db
    .prepare('SELECT * FROM transfer_credits WHERE plan_id = ? ORDER BY id')
    .all(req.params.id);
  res.json(rows);
});

// POST /api/plans/:id/transfer-credits — add a transfer credit
router.post('/:id/transfer-credits', (req, res) => {
  const db = getDb();
  const { name, code, equivalent_code, credits, institution } = req.body;
  if (!name || !code || !credits) return res.status(400).json({ error: 'name, code, and credits required' });
  const result = db
    .prepare('INSERT INTO transfer_credits (plan_id, name, code, equivalent_code, credits, institution) VALUES (?, ?, ?, ?, ?, ?)')
    .run(req.params.id, name, code, equivalent_code || null, credits, institution || 'Transfer Institution');
  res.status(201).json({ id: result.lastInsertRowid });
});

// DELETE /api/plans/:id/transfer-credits/:tcId
router.delete('/:id/transfer-credits/:tcId', (req, res) => {
  const db = getDb();
  db.prepare('DELETE FROM transfer_credits WHERE id = ? AND plan_id = ?').run(
    req.params.tcId,
    req.params.id
  );
  res.json({ ok: true });
